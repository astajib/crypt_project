...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("Crsfans_Busd_Dividend_Tracker", "Crsfans_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 3000000000 * (10**18); //must hold 3,000,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Crsfans_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Crsfans_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Crsfans contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Crsfans_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Crsfans_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Crsfans is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyTranscationAmount = 1000000 * (10**9);
    uint256 public maxSellTransactionAmount = 1000000 * (10**0);
    uint256 public maxWalletBalance = 1000000 * (10**9);
    uint256 public swapTokensAtAmount = 20 * 10**6 * 10**9;
    ...
    uint256 public previousStakeFee;

//add reward to wallet
    uint256 public totalFees = busdDividendRewardsFee.add(marketingFee).add(liquidityFee).add(charityFee).add(devFee).add(stakeFee);
    
    uint256 public _busdDividendRewardsFeeBuy = 9;
    uint256 public _marketingFeeBuy = 2;
    uint256 public _LpFeeBuy = 1;
    uint256 public _charityFeeBuy = 1.5;
    uint256 public _devFeeBuy = 2;
    uint256 public _stakeFeeBuy = 1.5;
    ...
    constructor() ERC20("Crsfans", "Crsfans") {
        	
    	marketingWallet = 0x5Fef60Dc24cB210E8Ba63f539ffB8C6e196313FF; 
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Crsfans: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "Crsfans: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "Crsfans: The new dividend tracker must be owned by the Crsfans token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Crsfans: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Crsfans: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Crsfans: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Crsfans: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Crsfans: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Crsfans: Trading has not started yet");
